...
resource "google_compute_instance" "vm_instance" {
  ...
  provisioner "local-exec" {
    ...
    echo -e "\n[all:vars]\nPKI_DIR=\"./pki\"\n" >> inv
    ansible-playbook -i inv local_playbook.yml --tags "pki,ssh" -e "gcp_ip_address=${google_compute_instance.vm_instance.network_interface.0.access_config.0.nat_ip}"
